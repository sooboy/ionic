...
class Animation {
    ...
    _asyncEnd(dur, shouldComplete) {
        util_1.assert(!this._unrgTrns, '_unrgTrns must be null');
        util_1.assert(!this._tm, '_tm must be null');
        util_1.assert(dur > 0, 'duration can not be 0 in async animations');
        const self = this;
    ...
    _progress(stepValue) {
        ...
        for (i = 0; i < effects.length; i++) {
            ...
            if (fx.from && fx.to) {
                ...
                util_1.assert(tweenEffect || !this._isAsync, 'in async animations to != from value');
                if (tweenEffect) {
                    this._twn = true;
                }
                if (stepValue === 0) {
