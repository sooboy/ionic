...
const virtual_util_1 = require("./virtual-util");
...
const virtual_item_1 = require("./virtual-item");
const virtual_footer_1 = require("./virtual-footer");
const virtual_header_1 = require("./virtual-header");
/**
 * @name VirtualScroll
 * @description
...
let VirtualScroll = class VirtualScroll {
    ...
    renderVirtual(needClean) {
        this._plt.raf(() => {
            ...
            this._zone.run(() => {
                virtual_util_1.populateNodeData(data.topCell, data.bottomCell, true, cells, records, nodes, this._itmTmp.viewContainer, this._itmTmp.templateRef, this._hdrTmp && this._hdrTmp.templateRef, this._ftrTmp && this._ftrTmp.templateRef, needClean);
            });
            if (needClean) {
            ...
            // ******** DOM READ ****************
            this._dom.read(() => virtual_util_1.initReadNodes(this._plt, nodes, cells, data));
            this._dom.write(() => {
                ...
                util_1.assert(this._queue === SCROLL_QUEUE_NO_CHANGES, 'queue value should be NO_CHANGES');
            });
        });
    }
    /**
    ...
    _stepNoChanges() {
        ...
        virtual_util_1.adjustRendered(cells, data);
        var hasChanges = virtual_util_1.populateNodeData(data.topCell, data.bottomCell, diff > 0, cells, records, nodes, this._itmTmp.viewContainer, this._itmTmp.templateRef, this._hdrTmp && this._hdrTmp.templateRef, this._ftrTmp && this._ftrTmp.templateRef, false);
        if (hasChanges) {
    ...
    scrollUpdate(ev) {
        ...
        if (queue === SCROLL_QUEUE_NO_CHANGES) {
            ...
            util_1.assert(queue === SCROLL_QUEUE_DOM_WRITE, 'queue value unexpected');
    ...
    scrollEnd() {
        ...
        this._dom.write(() => {
            // update the bound context for each node
            virtual_util_1.updateNodeContext(this._nodes, this._cells, this._data);
            // ******** DOM WRITE ***************
            this._stepChangeDetection();
            // ******** DOM WRITE ****************
            this._stepDOMWrite();
     ...
     */
    _listeners() {
        util_1.assert(!this._scrollSub, '_listeners was already called');
        if (!this._scrollSub) {
     ...
     */
    ngAfterContentInit() {
        util_1.assert(this._itmTmp, 'virtualItem required within virtualScroll');
...
__decorate([
    core_1.ContentChild(virtual_header_1.VirtualHeader),
    __metadata("design:type", virtual_header_1.VirtualHeader)
], VirtualScroll.prototype, "_hdrTmp", void 0);
__decorate([
    core_1.ContentChild(virtual_footer_1.VirtualFooter),
    __metadata("design:type", virtual_footer_1.VirtualFooter)
