...
class ViewController {
    ...
    dismiss(data, role, navOptions = {}) {
        if (!this._nav) {
            util_1.assert(this._state === nav_util_1.STATE_DESTROYED, 'ViewController does not have a valid _nav');
    ...
    _preLoad() {
        util_1.assert(this._state === nav_util_1.STATE_INITIALIZED, 'view state must be INITIALIZED');
     ...
     */
    _didLoad() {
        util_1.assert(this._state === nav_util_1.STATE_ATTACHED, 'view state must be ATTACHED');
     ...
     */
    _destroy(renderer) {
        util_1.assert(this._state !== nav_util_1.STATE_DESTROYED, 'view state must be ATTACHED');
        if (this._cmp) {
            if (renderer) {
