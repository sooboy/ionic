...
class ScrollView {
    ...
    init(ele, contentTop, contentBottom) {
        util_1.assert(ele, 'scroll-view, element can not be null');
        this._el = ele;
        if (!this.initialized) {
    ...
    enableNativeScrolling() {
        util_1.assert(this.onScrollStart, 'onScrollStart is not defined');
        util_1.assert(this.onScroll, 'onScroll is not defined');
        util_1.assert(this.onScrollEnd, 'onScrollEnd is not defined');
        this._js = false;
        if (!this._el) {
            return;
        }
