 ...
 */
let Range = Range_1 = class Range extends base_input_1.BaseInput {
    ...
    _pointerDown(ev) {
        ...
        const current = dom_1.pointerCoord(ev);
        ...
        const ratio = util_1.clamp(0, (current.x - rect.left) / (rect.width), 1);
    ...
    _pointerMove(ev) {
        ...
        ev.stopPropagation();
        // update the active knob's position
        const hasChanged = this._update(dom_1.pointerCoord(ev), this._rect, true);
    ...
    _update(current, rect, isPressed) {
        ...
        let ratio = util_1.clamp(0, (current.x - rect.left) / (rect.width), 1);
...
Range = Range_1 = __decorate([
    core_1.Component({
        ...
        providers: [{ provide: forms_1.NG_VALUE_ACCESSOR, useExisting: Range_1, multi: true }],
