...
class DeepLinker {
    ...
    initViews(segment) {
        ...
        return this.getNavLinkComponent(link).then((component) => {
            ...
            const view = new view_controller_1.ViewController(component, segment.data);
            view.id = segment.id;
            if (util_1.isArray(segment.defaultHistory)) {
                return nav_util_1.convertToViews(this, segment.defaultHistory).then(views => {
                    views.push(view);
                    return views;
                });
            }
    ...
    _loadViewForSegment(navContainer, segment, done) {
        ...
        if (nav_util_1.isTabs(navContainer) || (nav_util_1.isTab(navContainer) && navContainer.parent)) {
            const tabs = (nav_util_1.isTabs(navContainer) ? navContainer : navContainer.parent);
